  ...
  <div class="result-container">
    ...
    <div v-if="tripPlan" class="content-wrapper">
      ...
      <div class="side-nav">
        <a-affix :offset-top="80">
          <a-menu mode="inline" :selected-keys="[activeSection]" @click="scrollToSection">
            ...
            <a-menu-item key="knowledge" v-if="tripPlan.knowledge_references && tripPlan.knowledge_references.length > 0">
              <span>📚 知识参考</span>
            </a-menu-item>
          </a-menu>
        </a-affix>
      ...
      <div class="main-content">
        ...
        <a-card
          id="knowledge"
          v-if="tripPlan.knowledge_references && tripPlan.knowledge_references.length > 0"
          title="📚 旅行知识参考"
          :bordered="false"
        ...
        >
          <a-collapse>
            <a-collapse-panel
              v-for="(doc, idx) in tripPlan.knowledge_references"
              :key="doc.id"
              :header="doc.metadata?.title || '知识参考 #' + (idx + 1)"
            >
              <div class="knowledge-content">{{ doc.content }}</div>
              <div v-if="doc.score !== undefined" class="knowledge-score">
                相关度: {{ (doc.score * 100).toFixed(0) }}%
              </div>
              <div v-if="doc.metadata?.city" class="knowledge-meta">
                来源: {{ doc.metadata.source }} | 城市: {{ doc.metadata.city }}
              </div>
            </a-collapse-panel>
          </a-collapse>
        </a-card>
...
.map-card :deep(.ant-card-body) {
  height: 500px; /* 直接设置固定高度，或者确保 100% 生效 */
  padding: 0;
